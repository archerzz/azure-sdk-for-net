...
namespace Azure.ResourceManager.Network.Tests.Tests
{
    public class ExpandResourceTests : NetworkServiceClientTestBase
    {
        ...
        public async Task ExpandResourceTest()
        {
            ...
            foreach (InboundNatRule natRule in getLoadBalancer.Value.GetInboundNatRules().List())
            {
            ...
            foreach (BackendAddressPool pool in getLoadBalancer.Value.GetBackendAddressPools().List())
            {
                foreach (NetworkInterfaceIPConfiguration ipconfig in getLoadBalancer.Value.GetBackendAddressPools().List().First().Data.BackendIPConfigurations)
                {
            ...
            foreach (NetworkInterfaceIPConfiguration ipconfig in nic1.ListNetworkInterfaceIPConfigurations())
            {
