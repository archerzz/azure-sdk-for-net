    ...
    public class AvailableProvidersListTests : NetworkServiceClientTestBase
    {
        ...
        public async Task AvailableProvidersListAzureLocationCountrySpecifiedTest()
        {
            ...
            Operation<AvailableProvidersList> providersListOperation =
                await GetResourceGroup("NetworkWatcherRG").GetNetworkWatchers().Get("NetworkWatcher_westus").Value.StartListAvailableProvidersAsync(parameters);
        ...
        public async Task AvailableProvidersListAzureLocationCountryStateSpecifiedTest()
        {
            ...
            Operation<AvailableProvidersList> providersListOperation = await GetResourceGroup("NetworkWatcherRG").GetNetworkWatchers().Get("NetworkWatcher_westus").Value.StartListAvailableProvidersAsync(parameters);
            Response<AvailableProvidersList> providersList = await WaitForCompletionAsync(providersListOperation);
            Assert.AreEqual("United States", providersList.Value.Countries[0].CountryName);
