...
namespace Azure.ResourceManager.Network.Tests.Tests
{
    public class RouteFilterTests : NetworkServiceClientTestBase
    {
        ...
        public async Task RouteFilterApiTest()
        {
            ...
            var filters = await filterContainer.ListAsync().ToEnumerableAsync();
            Has.One.Equals(filters);
            Assert.AreEqual(filterName, filters[0].Data.Name);
            Assert.IsEmpty(filters[0].Data.Rules);

            var allFilters = await ArmClient.DefaultSubscription.ListRouteFiltersAsync().ToEnumerableAsync();
            ...
            var rules = await filter.GetRouteFilterRules().ListAsync().ToEnumerableAsync();
            Assert.IsEmpty(rules);

            // Delete filter
            await filter.DeleteAsync();
            allFilters = await ArmClient.DefaultSubscription.ListRouteFiltersAsync().ToEnumerableAsync();
