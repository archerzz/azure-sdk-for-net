...
namespace Azure.ResourceManager.Network.Tests.Tests
{
        ...
        : NetworkServiceClientTestBase
    {
        ...
        private Core.ResourceGroup resourceGroup;
        ...
        public async Task CleanupResourceGroup()
        {
            ...
            var ddosProtectionPlans = await GetContainer().ListAsync().ToEnumerableAsync();
        ...
        public async Task DdosProtectionPlanApiTest()
        {
            ...
            // create
            var ddosProtectionPlanResponse = await container.CreateOrUpdateAsync(name, TestEnvironment.Location);

            Assert.True(await container.DoesExistAsync(name));
            ...
            // update
            ddosProtectionPlanResponse = await container.CreateOrUpdateAsync(name, TestEnvironment.Location, new Dictionary<string, string> {
                {"tag1", "value1" },
                {"tag2", "value2" }
            });
            ...
            // patch
            ddosProtectionPlanResponse = await ddosProtectionPlanResponse.Value.RemoveTagAsync("tag1");
            ddosProtectionPlanData = ddosProtectionPlanResponse.Value.Data;
            ...
            // list
            var ddosProtectionPlans = await container.ListAsync().ToEnumerableAsync();
            ...
            Assert.False(await container.DoesExistAsync(name));

            ddosProtectionPlans = await container.ListAsync().ToEnumerableAsync();
            Assert.IsEmpty(ddosProtectionPlans);

            // list all
            // TODO: ADO 6080
            //ddosProtectionPlans = await ArmClient.DefaultSubscription.ListDdosProtectionPlanAsync();
            //Assert.IsEmpty(ddosProtectionPlans);
        }
